...
const KICKOFF_TEXT =
  "(Sesi dimulai — sapa kandidat dengan singkat lalu ajukan pertanyaan pertamamu.)";
...
export function useInterviewSession(sessionId: string) {
  ...
  // Otomatis mute mic saat AI sedang bicara, dan aktifkan kembali saat AI selesai bicara
  // (kecuali jika user sendiri yang men-mute secara manual).
  useEffect(() => {
    mic.setMuted(userMuted || interviewerSpeaking);
  }, [userMuted, interviewerSpeaking, mic]);
  ...
  const start = useCallback(async () => {
    ...
    try {
      const { data } = await api.post<TokenResponse>(
        `/api/interview/sessions/${sessionId}/token`,
      );
        ...
        callbacks: {
          ...
          onOutputTranscript: (text) => {
            ...
            if (
              ...
              INTERVIEW_CLOSING_PHRASES.some((p) => spoken.includes(p))
            ) {
              closingDetectedRef.current = true;
            }
          },
          onTurnComplete: async () => {
            // Tunggu hingga antrean suara AI di speaker selesai terputar penuh baru un-mute mic
            await playback.waitUntilDrained(10000);
      ...
      timerRef.current = setInterval(() => {
        ...
        if (elapsed >= maxDurationRef.current && !endingRef.current)
  ...
  return {
    ...
    muted: userMuted || interviewerSpeaking,
    userMuted,
    setMuted: (val: boolean) => setUserMuted(val),
